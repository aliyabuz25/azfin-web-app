          ...
          <div className="flex flex-col md:flex-row items-end justify-between gap-10">
            <div className="max-w-2xl">
              <div className="flex items-center gap-2 text-accent mb-6 text-[10px] font-bold uppercase tracking-[0.4em]">
                ...
                {siteSettings.uiServiceCatalogue || 'Xidmət Kataloqumuz'}
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-primary tracking-tight leading-tight uppercase italic">
                {siteSettings.uiServiceHeader1 || 'Biznesiniz üçün'} <br />
                <span className="text-accent">{siteSettings.uiServiceHeader2 || 'İxtisaslaşmış Həllər'}</span>
              </h1>
            </div>
            <p className="text-slate-500 font-bold text-xs max-w-xs border-l-2 border-slate-200 pl-6 pb-2 uppercase tracking-widest italic">
              {siteSettings.uiServiceSub || 'Azfin Consulting hər bir müəssisəyə fərdi yanaşaraq, maliyyə dayanıqlığını təmin edən dəqiq strategiyalar hazırlayır.'}
            </p>
          </div>
        </div>
      </div>
