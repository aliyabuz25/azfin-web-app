...
const ApplicationModal: React.FC<ApplicationModalProps> = ({ isOpen, onClose, trainingTitle }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-brand/60 backdrop-blur-md transition-opacity" onClick={handleClose}></div>
      ...
      <div className="relative bg-white w-full max-w-lg rounded-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
          ...
          {/* Header */}
          <div className="bg-brand px-6 py-5 flex justify-between items-center border-b border-accent/30">
          ...
          <div className="p-8">
            {submitted ? (
                <div className="text-center py-6 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <h3 className="text-xl font-black text-brand uppercase italic tracking-tight mb-3">Müraciətiniz qəbul olundu!</h3>
                    ...
                    <button 
                        onClick={handleClose}
                        className="bg-brand text-white px-10 py-3 rounded-full font-bold text-[9px] uppercase tracking-[0.3em] hover:bg-brand-medium transition-all"
                    >
                        Bağla
                    </button>
                </div>
                ...
                <div className="space-y-6">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 border-l-2 border-accent pl-4">
                      Siz <span className="text-brand">{trainingTitle}</span> üzrə qeydiyyat formunu doldurursunuz.
                    </p>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div className="pt-4">
                            <button 
                                type="submit"
                                className="w-full bg-accent text-white py-5 rounded-sm font-black text-[11px] uppercase tracking-[0.2em] hover:bg-brand-medium transition-all shadow-xl"
                            >
                                Müraciət Et
                            </button>
                        </div>
