...
const CalculationModal: React.FC<CalculationModalProps> = ({ isOpen, onClose, serviceType, serviceTitle }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-brand/60 backdrop-blur-md transition-opacity" onClick={onClose}></div>
      <div className={`relative w-full max-w-2xl rounded-sm transition-all duration-500 overflow-hidden ${submitted ? 'bg-transparent shadow-none max-w-sm' : 'bg-white shadow-2xl'}`}>
        {!submitted && (
          <div className="bg-brand p-6 flex justify-between items-center border-b border-accent/30">
            <div>
          ...
          {submitted ? (
            <div className="flex flex-col items-center justify-center animate-in fade-in zoom-in duration-500">
              <div className="bg-white/90 backdrop-blur-xl p-12 rounded-3xl border border-white/20 shadow-2xl text-center w-full">
                ...
                <h4 className="text-brand font-extrabold text-sm uppercase tracking-[0.2em] mb-3 italic">Təşəkkür edirik!</h4>
                ...
                <button onClick={onClose} className="bg-brand text-white px-10 py-3 rounded-full font-bold text-[9px] uppercase tracking-[0.3em] hover:bg-brand-medium transition-all shadow-md">Tamam</button>
              </div>
            </div>
          ) : (
            <div className="p-8 md:p-12">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="pt-4">
                  <button type="submit" className="w-full bg-accent text-white py-5 rounded-sm font-black text-[11px] uppercase tracking-[0.2em] hover:bg-brand-medium transition-all shadow-xl flex items-center justify-center gap-3">
