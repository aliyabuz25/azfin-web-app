...
const Navbar: React.FC = () => {
  ...
  const NAV_ITEMS = [
    { label: siteSettings.uiHome || 'ANA SƏHİFƏ', path: '/' },
    { label: siteSettings.uiAbout || 'HAQQIMIZDA', path: '/about' },
    { label: siteSettings.uiServices || 'XİDMƏTLƏR', path: '/services', children: true },
    { label: siteSettings.uiAcademy || 'AKADEMİYA', path: '/academy' },
    { label: siteSettings.uiBlog || 'BLOQ', path: '/blog' },
    { label: siteSettings.uiAuditTV || 'AUDİTTV', path: siteSettings.auditTvUrl, isExternal: true },
  ];
                      ...
                      {activeDropdown === item.label && (
                        <div className="absolute top-[100%] left-1/2 -translate-x-1/2 w-[550px] bg-white border border-slate-100 shadow-2xl p-6 grid grid-cols-2 gap-2 animate-in fade-in slide-in-from-top-1">
                          {services.map((service) => {
                            const SvgIcon = getIcon(service.iconName || '');
                            return (
                              <Link
                                key={service.id}
